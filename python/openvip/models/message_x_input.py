...
class MessageXInput(BaseModel):
    ...
    """ # noqa: E501
    ops: Annotated[List[StrictStr], Field(min_length=1)] = Field(description="Ordered list of input operations to perform")
    ...
    __properties: ClassVar[List[str]] = ["ops", "trigger", "confidence", "source"]

    @field_validator('ops')
    def ops_validate_enum(cls, value):
        """Validates the enum"""
        for i in value:
            if i not in set(['submit', 'newline']):
                raise ValueError("each list item must be one of ('submit', 'newline')")
        return value
